...
export class APIFavoriteArticle {
	...
	constructor({ articleService, userService }: APIFavoriteArticleConstructor) {
		this.articleService = articleService;
		this.userService = userService;
	}

	async execute({ slug, user }: APIFavoriteArticleInput): Promise<APIFavoriteArticleOutput> {
		try {
			const article = await this.articleService.getArticleBySlug({ slug });
			await this.articleService.favorite({ userId: user.id, articleId: article.id });
