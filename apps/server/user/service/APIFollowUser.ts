...
export class APIFollowUser {
	 ...
	 *
	 */
	async execute({ user, username }: APIFollowUserInput): Promise<APIFollowUserOutput> {
		...
		try {
			await this.userService.followUser({ followerId: user.id, followingId: targeted.id });
			const profile = new DtoProfile({ dbDtoUser: targeted, following: true });
			return { profile };
		} catch (error) {
