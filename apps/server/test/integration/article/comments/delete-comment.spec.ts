...
import { getCreateArticleInput, getCreateCommentInput, getCreateUserInput } from "@conduit/core/test/mockData";
import { app } from "../../../../app";
...
describe("Article - Delete Comment", () => {
	it("should be able to delete comments from an article", async () => {
		...
		const response = await request
			.delete(ServerPath.DeleteComment.replace(":slug", article.slug).replace(":id", comment.id))
	...
	it("should return a status code of 404 - Not Found if the targeted article's comment does not exist", async () => {
		...
		const response = await request
			.delete(ServerPath.DeleteComment.replace(":slug", article.slug).replace(":id", "NON_EXIST_ID"))
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
				comment: {
					body: faker.lorem.sentence()
...
const setup = async () => {
	...
	const comment = await articleService.createArticleComment(getCreateCommentInput({ articleId: article.id, userId: user.id }));
	const { accessToken } = signJsonWebToken({ dbDtoUser: user });
	return {
		userService,
		articleService,
