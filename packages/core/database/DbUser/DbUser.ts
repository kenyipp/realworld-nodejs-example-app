...
import {
	...
	FollowUserInput,
	FollowUserOutput,
	GetIsUsersFollowingByUserIdsInput,
	GetIsUsersFollowingByUserIdsOutput,
	GetUserByEmailInput,
	...
	HiddenUserContentInput,
	HiddenUserContentOutput,
	IsFollowingInput,
	IsFollowingOutput,
	IsUserExistInput,
	IsUserExistOutput,
...
export class DbUser {
	 ...
	 *
	 */
	async getUserByIds({ ids }: GetUserByIdsInput): GetUserByIdsOutput {
		if (ids.length < 1) {
			return [];
		}
	 ...
	 *
	 * @returns {Promise<DbDtoUser|undefined>} - A promise that resolves to the user object if found, or null if not found.
	 *
	 */
	 ...
	 * @param {string} input.email - The email address of the user to retrieve.
	 *
	 * @returns {Promise<DbDtoUser|undefined>} - A promise that resolves to the user object if found.
	 *
	 */
	async getUserByUsername({
		username
	}: GetUserByUsernameInput): GetUserByUsernameOutput {
		const { id } = await knex
			.first<{ id: string }>({ id: "user_id" })
			.from(Tables.User)
	...
	async isUserExist({
		username,
		email
	}: IsUserExistInput): IsUserExistOutput {
		if (!username && !email) {
			return false;
		}
		let query = knex
	...
	async isFollowing({
		followerId,
		followingId
	}: IsFollowingInput): IsFollowingOutput {
		const id = await knex
	 ...
	 *
	 * @returns {Promise<Object>} A promise that resolves with an object mapping the given user IDs to boolean values indicating whether or not they are being followed.
	 * @throws {Error} If the database query fails for any reason.
	 *
	 */
	async getIsUsersFollowingByUserId({
		followerId,
		followingIds
	}: GetIsUsersFollowingByUserIdsInput): GetIsUsersFollowingByUserIdsOutput {
	 ...
	 *
	 */
	async banUserById({ id }: BanUserByIdInput): BanUserByIdOutput {
		await knex
			.table(Tables.User)
	 ...
	 *
	 */
	async hiddenUserContent({
		userId
	}: HiddenUserContentInput): HiddenUserContentOutput {
		await knex
