...
describe("Article - Favorite", () => {
	describe("Favorite an Article", () => {
		it("should be able to favorite an article", async () => {
			const { articleService, user, article } = await setup();
			await articleService.favorite({
				userId: user.id,
				articleId: article.id
			});
			const isFavorited = await articleService.isArticleFavorited({
				userId: user.id,
				articleId: article.id
			});
	...
	describe("Unfavorite an Article", () => {
		it("should be able to unfavorite an article", async () => {
			...
			isFavorited = await articleService.isArticleFavorited({
				userId: user.id,
				articleId: article.id
			});
			expect(isFavorited).to.be.true;
