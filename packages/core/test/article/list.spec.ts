...
describe("Article - List Articles", () => {
	it("should be able to return a list of articles", async () => {
		...
		const articles = await articleService.getArticlesByFilters({ offset: 0, limit: 10 });
	...
	it("should be able to return a filtered article list by tags", async () => {
		...
		const tag = "react.js";
		await articleService.createArticleTag({ articleId: articleA.id, tagList: ["react.js"] });
		const articles = await articleService.getArticlesByFilters({ offset: 0, limit: 10, tag });
	...
	it("should be able to return a filtered article list by the author's name", async () => {
		...
		articles = await articleService.getArticlesByFilters({ offset: 0, limit: 10 });
		expect(articles).have.lengthOf(3);
		expect(articles[0].id).equals(article.id);
		articles = await articleService.getArticlesByFilters({ offset: 0, limit: 10, author: user.username });
		expect(articles).have.lengthOf(1);
		expect(articles[0].id).equals(article.id);
	});
	...
	it("should return an empty list if no articles match the filter parameters", async () => {
		...
		const articles = await articleService.getArticlesByFilters({ offset: 0, limit: 10, tag: "react.js" });
		expect(articles).have.lengthOf(0);
	});

	it("should be able to limit the number of articles returned", async () => {
		const { articleService, articleB } = await setup();
		const articles = await articleService.getArticlesByFilters({ offset: 0, limit: 1 });
		expect(articles).have.lengthOf(1);
		expect(articles[0].id).equals(articleB.id);
	});
	...
	it("should be able to paginate the articles using the offset parameter", async () => {
		...
		const articles = await articleService.getArticlesByFilters({ offset: 1, limit: 1 });
		expect(articles).have.lengthOf(1);
		expect(articles[0].id).equals(articleA.id);
	});
	...
	it("should be able to return a list of articles by followed users, ordered by most recent first", async () => {
		...
		articles = await articleService.getArticlesByFilters({ followedBy: userA.id, offset: 0, limit: 10 });
		...
		await articleService.createArticle(getCreateArticleInput({ userId: userB.id }));
	...
	it("should be able to remove articles from the feed when the user unfollows another user", async () => {
		...
		await userService.unfollowUser({ followerId: userA.id, followingId: userB.id });
	...
	const setup = async () => {
		...
		const user = await userService.createUser(getCreateUserInput({}));
		const articleA = await articleService.createArticle(getCreateArticleInput({ userId: user.id }));
