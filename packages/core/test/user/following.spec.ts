...
describe("User - following", () => {
	describe("Follow an User", () => {
		it("should be able to follow an user", async () => {
			...
			const isFollowing = await userService.isFollowing({ followerId: userA.id, followingId: userB.id });
		...
		it("should throw an error if the user is trying to follow themselves", async () => {
			...
			try {
				await userService.followUser({ followerId: userA.id, followingId: userA.id });
				expect.fail();
			} catch (error) {
				expect(error).instanceOf(InvalidFollowError);
			}
	...
	describe("Unfollow an User", () => {
		it("should be able to unfollow an user", async () => {
			...
			await userService.followUser({ followerId: userA.id, followingId: userB.id });
			isFollowing = await userService.isFollowing({ followerId: userA.id, followingId: userB.id });
			expect(isFollowing).to.be.true;
